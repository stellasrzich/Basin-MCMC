...
if they produce similar data (say, if we treat the data generated by the representative state as true data, 
then the likelihood \hl{(or, posterior probability density?)} of the data generated by the state is high).
...
\begin{itemize}
    ...
    \hl{does it generate samples from the basin probability distribution, or the posterior?}
\end{itemize}

Recall that out original MCMC algorithm worked by doing a mixture of moves between states of the model, 
...
\begin{itemize}
    ...
    \hl{Would we ever switch the representative state? Maybe at the end of a peturb move?}
