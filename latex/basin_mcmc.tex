...
\begin{algorithm}[H]
  ...
  initial basin probability distribution $\pi_{B,0}(\cdot)$ (dependent on representative state $x_0$), 
...
\begin{algorithmic}[1]
  \State Initialize list of basins $\mathcal{B}^0 = [B]$ with $B= (x_0, \pi_{B,0}(\cdot), p_{B,0} = 1)$.
  \For{$j = 0$ to $N-1$}
    ...
    \ElsIf{Perturb:}
      ...
      $\mathcal{B}^*$ is $\mathcal{B}^j$ with probability distribution adapted and a different representative state. 
      This is done by running the local sampler within the basin.
